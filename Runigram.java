...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i =0; i< image.length; i++){
			for (int j =0; j< image[0].length; j++){
				horizontal[i][image[0].length-(j+1)]= image[i][j];
			}
		}
	...
	public static Color[][] flippedVertically(Color[][] image){
		Color [][] vertical = new Color[image.length][image[0].length];
			for (int j =0; j< image[0].length; j++){
				for (int i =0; i< image.length; i++){
				vertical[image.length-(i+1)][j]= image[i][j];
			}
		}
		return vertical;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		Color [][] changeSize = new Color[height][width];
		int h0 = image.length;
		...
		for (int i =0; i< height; i++){
			for (int j =0; j< width; j++){
				changeSize[i][j]= image[i* h0 / height][j* w0 / width];
			}
		}
		return changeSize;
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		int finalRed = (int) (alpha * rC1 + (1-alpha) * rC2) ;
		int finalGreen = (int) (alpha * gC1 + (1-alpha) * gC2) ;
		int finalBlue = (int) (alpha * bC1 + (1-alpha) * bC2) ;


		Color blended = new Color(finalRed, finalGreen,finalBlue );
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for (int i =0; i< source.length; i++){
			for (int j =0; j< source[0].length; j++){
				while (n>=i){
				source[i][j]= blend(source[i][j], target[i][j], (n-i)/n);
